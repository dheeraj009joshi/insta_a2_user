...
from hikerapi import Client as clust
import json
...
cl=clust(token="q8pw5b23vrgt2f50vxgdyblrg5k763yv")
MAX_USER_COUNT = 200
...
class UserMixin:
    ...
    def user_info_by_username_a2(self, username: str) -> User:
        """
        Get user object from user name using a2
        ...
        -------
        User
            An object of a2 response 
        """
        username = str(username).lower()
        try:
            result = self.private_request(f"a2/user/{username}")
        except ClientNotFoundError as e:
            raise UserNotFound(e, username=username, **self.last_json)
        except ClientError as e:
            if "User not found" in str(e):
                raise UserNotFound(e, username=username, **self.last_json)
            raise e
        return extract_user_a2(result["user"])
    ...
    def user__info_by_username_a2(self, username: str) -> User:
        """
        Get user object from user name using a2
